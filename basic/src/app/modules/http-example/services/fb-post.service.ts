...
export class FBPostService {
  ...
  savePost(post: Post): void {
    this.http.post<{ id: string }>(this.firebaseURL, post, { observe: 'response' }).subscribe(
    ...
    this.http
      ...
      .pipe(
        tap((event: HttpEvent<null>) => {
          // console.log(event)
          if (event.type === HttpEventType.Sent) console.log('request sent', event)

          if (event.type === HttpEventType.Response) console.log('response received', event)
        })
      )
  ...
  getPosts(): Observable<Post[]> {
    return this.http
      .get<{ [key: string]: Post }>(this.firebaseURL, {
        ...
        params: new HttpParams().append('print', 'pretty').appendAll({
          'test-param': 'test',
          'test-param-2': 'test-2'
        })
      })
